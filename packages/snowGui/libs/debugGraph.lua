...
function debugGraph:new(type, x, y, width, height, delay, label, font)
    ...
    local instance = {
        x = x or 0,            -- X position
        y = y or 0,            -- Y position
        width = width or 40,   -- Graph width
        height = height or 20, -- Graph height
        delay = delay or 0.1,  -- Update delay
    ...
    function instance:update(dt, val)
        ...
        if dt > self.delay or lastTime > self._time then
            ...
            table.insert(self.data, val * 100) --dt数值波动
...
function debugGraph:load(...)
    self.fpsGraph = debugGraph:new('fps', 0, 0)
    self.memGraph = debugGraph:new('mem', 0, 30)
    self.dtGraph = debugGraph:new('custom', 0, 60)
end

function debugGraph.round(n, deci)
    deci = 10 ^ (deci or 0)
    return math.floor(n * deci + 0.5) / deci
end

function debugGraph:update(dt)
    self.fpsGraph:update(dt)
    self.memGraph:update(dt)

    -- Update our custom graph
    self.dtGraph:update(dt, math.floor(dt * 1000))
    self.dtGraph.label = 'DT: ' .. self.round(dt, 4)
end

function debugGraph:draw()
    self.fpsGraph:draw()
    self.memGraph:draw()
    self.dtGraph:draw()
end
